...
const TABS = [
  { id: 'general', icon: Settings, labelKey: 'sett.tabGeneral' },
  { id: 'balance', icon: Wallet, labelKey: 'sett.tabBalance' },
  { id: 'api', icon: Key, labelKey: 'sett.tabApi' },
];
...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState(
    TABS.some(tab => tab.id === tabFromUrl) ? tabFromUrl : 'general'
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Tabs */}
      <div className={`flex gap-1 p-1 rounded-xl ${isDark ? 'bg-[#111113] border border-[#1f1f23]' : 'bg-gray-100 border border-gray-200'}`}>
        {TABS.map((tab) => {
