...
import {
  Home, BarChart3, CreditCard, Users,
  BookOpen, Link2, Settings, LogOut, Zap, Bot, Key
} from 'lucide-react';
...
export default function Sidebar({ onLogout, user }) {
  ...
  const menuItems = [
    ...
    { path: '/balance', icon: CreditCard, label: t('sidebar.balance') },
    ...
    { path: '/api-settings', icon: Key, label: t('sidebar.apiSettings') },
  ...
  return (
    <aside className={`w-72 h-screen sticky top-0 border-l flex flex-col transition-colors duration-300 ${isDark ? 'bg-[#111113] border-[#1f1f23]' : 'bg-white border-gray-200'}`}>
        ...
        {menuItems.map((item) => (
          <button key={item.path} onClick={() => navigate(item.path)}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${
              isActive(item.path) ? 'bg-teal-500/10 text-teal-500 border border-teal-500/20'
      ...
      <div className={`p-4 border-t transition-colors shrink-0 ${isDark ? 'border-[#1f1f23]' : 'border-gray-200'}`}>
        <div className={`flex items-center gap-3 p-3 rounded-xl ${isDark ? 'bg-[#0a0a0b]' : 'bg-gray-50'}`}>
          ...
          <button onClick={onLogout} className={`p-2 rounded-lg transition-colors shrink-0 ${isDark ? 'hover:bg-[#1a1a1d] text-gray-400' : 'hover:bg-gray-200 text-gray-500'}`} title={t('sidebar.logout')}>
